...
const DOWFULL = ['Sundays', 'Mondays', 'Tuesdays', 'Wednesdays', 'Thursdays', 'Fridays', 'Saturdays'];
...
export function wrappedSvg(d, opts = {}) {
  ...
  const compact = (x, y, w, accent, emoji, title, value) =>
    ...
    txt(x + 62, y + 54, 16, c.text, clip(value, Math.floor((w - 74) / 8.4)), 'font-weight="600"');
  ...
  const msgCard = (y, accent, emoji, title, quote, meta) =>
    ...
    txt(P + 66, y + 60, 16.5, c.text, '“' + clip(quote, 62) + '”', 'font-weight="600"') +
    ...
    parts.push(
      tile(
        rectEl(P, y, IW, h, c.tile, c.border) +
          ...
          txt(W - P - 18, y + 28, 13, c.sub, `≈ ${num(d.avgPerDay)}/day`, 'text-anchor="end"') +
          (series.length > 1
  ...
  // Weekly + daily rhythm (two half cards)
  {
    ...
    const weekly =
      ...
      txt(P + 18, y + 122, 12.5, c.sub, `Most alive on ${DOWFULL[d.busiestDow]}`, 'font-weight="600"');
    const x2 = P + hw + G;
    const hours = d.hour || [];
    ...
    const daily =
      ...
      axis +
      txt(x2 + 18, y + 122, 12.5, c.sub, `${persona(t.peakHour)} · peak ${hourLabel(t.peakHour)}`, 'font-weight="600"');
  ...
  if (d.top3 && d.top3.length) {
    const h = 116;
    const medals = ['🥇', '🥈', '🥉'];
    let s = rectEl(P, y, IW, h, c.tile, c.border) + txt(P + 18, y + 28, 13, c.muted, 'Top chatters');
    for (let i = 0; i < Math.min(3, d.top3.length); i++) {
      ...
      s +=
        ...
        txt(cx, y + 90, 15, c.text, clip(u.name, 16), 'text-anchor="middle" font-weight="600"') +
        txt(cx, y + 108, 12, c.muted, `${num(u.count)} msgs`, 'text-anchor="middle"');
    }
    parts.push(tile(s));
    y += h + G;
  }
  ...
  if (d.top3Emoji && d.top3Emoji.length) {
    const h = 62;
    const items = d.top3Emoji.map((e) => `${e.name}  ${num(e.count)}`).join('     ');
    parts.push(
      tile(
        rectEl(P, y, IW, h, c.tile, c.border) +
  ...
  // 2×2 compact highlights
  {
    ...
    if (d.busiest)
      cells.push((x, yy) =>
        compact(x, yy, hw, c.warning, '📈', 'Busiest day', `${fmtDayKey(d.busiest.date)} · ${num(d.busiest.count)}`),
      );
    if (d.topPair)
      cells.push((x, yy) =>
        compact(x, yy, hw, c.accent, '💞', 'Top duo', `${d.topPair.from} ↔ ${d.topPair.to} · ${num(d.topPair.count)}`),
      );
    if (d.starter)
      cells.push((x, yy) =>
        compact(x, yy, hw, c.success, '🗣️', 'Conversation starter', `${d.starter.name} · ${num(d.starter.count)} days`),
      );
    if (d.nightOwl)
      cells.push((x, yy) =>
        compact(x, yy, hw, c.danger, '🦉', 'Night owl', `${d.nightOwl.name} · ${num(d.nightOwl.count)} late msgs`),
      );
    let s = '';
    cells.forEach((fn, i) => {
      const x = P + (i % 2) * (hw + G);
